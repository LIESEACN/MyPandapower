...
def mpc2pp(dirpath):
    '''
    mpc2ppc: convert mpc to pandapower format
    dirpath: location of your mpc.xlsx
    '''
    ...
    baseMVA = 100
    hz = 50
    mycase = pp.create_empty_network(sn_mva=baseMVA, f_hz=hz)
    omega = 2 * np.pi * hz

    # bus create
    for i in range(bus.shape[0]):
        pp.create_bus(mycase, vn_kv=bus.loc[i, 'baseKV'], name='bus'+str(i+1),
    ...
    for i in range(gen.shape[0]):
        ...
        pp.create_gen(
            mycase, bus=gen.loc[i, 'bus']-1, p_mw=gen.loc[i, 'Pg'],
    ...
    for i in range(branch.shape[0]):
        ...
        if (branch.loc[i, 'ratio'] == 0) & (fv == tv):
            ...
            pp.create_transformer_from_parameters(
                mycase, hv_bus=h_bus, lv_bus=l_bus,
